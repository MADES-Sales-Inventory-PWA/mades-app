...
import { getProductBySku, type ProductSize, type SizeType } from "../../data/products";
...
export default function ProductEditPage() {
  ...
  const [form, setForm] = useState({
    ...
    tipoTalla: (product?.tipoTalla ?? "letras") as SizeType,
  });
  const [sizes, setSizes] = useState<ProductSize[]>(
    product?.tallasRegistradas ?? [{ talla: "", cantidadExistente: 0, unidadesAlerta: 0 }]
  );
          ...
          <section className="mx-auto flex w-full max-w-[1000px] flex-col gap-4 p-4 lg:p-6">
            ...
            <form
              ...
              onSubmit={(event) => {
                ...
                navigate(`/productos/${form.sku}${selectedSize ? `?talla=${encodeURIComponent(selectedSize)}` : ""}`);
              }}
            >
              ...
              <div className="flex justify-end gap-2">
                <button
                  type="button"
                  onClick={() => navigate(`/productos/${product.sku}`)}
